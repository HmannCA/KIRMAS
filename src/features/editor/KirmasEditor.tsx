...
export default function KirmasEditor(){
  ...
  // *** Debug: immer loggen, wenn sich der Editor-Schema-State ändert
  React.useEffect(() => {
    console.log('[Editor] schema updated:', {
      title: schema?.title,
      pages: Array.isArray(schema?.pages) ? schema.pages.length : 'n/a'
    });
    // Tastprobe im Browser möglich:
    (window as any).__KIRMAS_EDITOR_SCHEMA = schema;
  }, [schema]);
  ...
  return (
    <DndContext sensors={sensors} collisionDetection={closestCenter} onDragEnd={onDragEnd} >
        ...
        <div>
          <div className="row" style={{ justifyContent:'space-between', margin:'8px 0' }}>
            <div className="row" style={{ gap:8 }}>
              ...
              <EditorSaveToolbar getSchema={() => schema} lastId={currentId} lastTitle={schema?.title} onSaved={(info)=>{ setCurrentId(info.id); }} />
            </div>
          </div>
      ...
      <AiPanel
        ...
        getSchema={() => schema}
        onApply={(next, mode) => {
          console.log('[Editor] onApply received', { mode, pages: Array.isArray(next?.pages) ? next.pages.length : 'n/a', title: next?.title });
          (window as any).__KIRMAS_LAST_APPLY = { mode, next };
          // Falls du ensureIds nutzt, gern hier:
          const withIds = /* ensureIds ? */ next /* : next */;
          setSchema(withIds);
          setSelected(null);
        }}
        onLoad={({ id, title }) => {
          console.log('[Editor] onLoad', { id, title });
          setCurrentId(id);
        }}
      />
    </DndContext>
